import machine, sdcard, os, time, I2C, Pin
import utime
...
# ─── Montar SD ───────────────────────────────────────────────────
    ...
    for i in range(5):
        try:
            ...
            print("Error montando SD (intento {}): {}".format(i+1, e))
            time.sleep(1)
            montada = False
    
    # obtener fecha y hora actual (simulado aquí, en práctica usar RTC o NTP)
    fecha = "2024-06-01"
    hora = "12:00:00"
...
# ─── crear archivo de registro ────────────────────────────────────────────────
    escribir(f"/sd/{fecha}.txt", "Fecha,Hora,Temperatura(°C),Humedad(%),Presion(hPa),precipitacion(mm),lluvia acumulada(mm),velocidad del viento (km/h),direccion del viento\n")      
...
# ─── Datalogger continuo (cada 10 segundos) ──────────────────────
    ...
    mm = pulsos_ll * MM_POR_PULSO
    mm_acumulada = acumulada * MM_POR_PULSO
    pulsos = 0
